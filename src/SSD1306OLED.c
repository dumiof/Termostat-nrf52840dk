...
void ssd1306_command(uint8_t c)
{
    ...
    temp[0] = control;
    temp[1] = c;
    ret = i2c_write_dt(_dev_i2c, temp, sizeof(temp));
    if(ret != 0){
...
void SSD1306_Display(void)
{
  ...
  for (uint16_t i = 0; i < (SSD1306_LCDWIDTH*SSD1306_LCDHEIGHT / 8)/32; i++) {
      ...
      //i--;
      //I2C_Stop(SSD1306_STREAM);
      memcpy(&temp[1], &ssd1306_buffer[i*32], 32);
      ret = i2c_write_dt(_dev_i2c, temp, sizeof(temp));
      if(ret != 0){
        printk("Failed to write to I2C device address %x \n", _dev_i2c->addr);
        return -1;
